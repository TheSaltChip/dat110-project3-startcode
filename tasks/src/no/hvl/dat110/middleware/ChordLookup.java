...
public class ChordLookup {
    ...
    private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
        ...
        for (int i = nodes.size() - 1; i >= 0; i--) {
            stub = Util.getProcessStub(nodes.get(i).getNodeName(), nodes.get(i).getPort());
            if (Util.computeLogic(stub.getNodeID(),
                    node.getNodeID().add(BigInteger.ONE),
    ...
    public void notify(NodeInterface pred_new) throws RemoteException {
        ...
        if (pred_old == null) {
            ...
            if (cond) {
                System.out.println(pred_new.getNodeName() + " " + pred_newID + "\n" + pred_old.getNodeName() + " " + pred_oldID + "\n" + node.getNodeName() + " " + nodeID + '\n');
                node.setPredecessor(pred_new);        // accept the new predecessor
            }
        }
    }
